...
// Add dcsOther() here if using existing first-party cookie, or dcsPlugin() here if using WT Cookie Plugin
function dcsOther(){
  var CHINAAMC_COOKIE = abcGetCookie("CHINAAMC_COOKIE");
  //alert(AbcUserName);
  //AbcUserName = gI18n?dcsEscape(dcsEncode(AbcUserName),I18NRE):AbcUserName;
  //alert(AbcUserName)
  //WT.dcsvid = AbcUserName;
  //alert(CHINAAMC_COOKIE);
  DCSext.CHINAAMC_COOKIE = CHINAAMC_COOKIE;
  if (typeof(gFpc)!="undefined"){
    dcsFPC(gTimeZone);
  }
}
// ABChina utility function called by getCookie()
function abcGetCookieVal(offset) {
...
// ABChina primary function to retrieve cookie by name
